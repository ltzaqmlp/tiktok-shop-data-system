...
public class ModuleRegistrar implements ApplicationRunner {
    ...
    @Override @Transactional public void run(ApplicationArguments ignored){
        ...
        String[][] menus={{"dashboard","dashboard.overview","数据看板","/dashboard","dashboard/index","DataBoard"},{"daily","daily.report","日报","/daily-reports","daily/reports","Document"},{"shooting","shooting.ticket","拍摄工单","/shooting-tickets","shooting/tickets","Document"},{"import","import.upload","数据导入","/data-import","import/index","Upload"},{"admin","admin.group","系统管理",null,null,"Menu"},{"admin","admin.users","用户管理","/admin/users","admin/users","User"},{"admin","admin.roles","角色管理","/admin/roles","admin/roles","Key"},{"admin","admin.menus","菜单管理","/admin/menus","admin/menus","Menu"},{"admin","admin.sku-config","SKU配置","/admin/sku-config","admin/sku-config","Goods"},{"admin","admin.audit","审计日志","/admin/audit","admin/audit","Document"},{"admin","admin.login-logs","登录日志","/admin/login-logs","admin/login-logs","Connection"},{"export","export.details","明细导出",null,null,"Document"}};
        int order=100;
        for(var m:menus){
        ...
        db.exec("insert into sys_role_menu(role_id,menu_id) select r.id,m.id from sys_role r cross join sys_menu m where r.role_code='ADMIN' and m.menu_code='admin.sku-config' on conflict do nothing",Map.of());
        boolean bootstrap=db.count("select count(*) from sys_user",Map.of())==0;
        if(bootstrap){
        ...
        if(bootstrap){
            ...
            db.exec("insert into sys_role_menu(role_id,menu_id) select r.id,m.id from sys_role r cross join sys_menu m where r.role_code='ADMIN' or (r.role_code in ('BOSS','DEPT_HEAD','OPS','ADS_BUYER','DIRECTOR') and m.menu_code='dashboard.overview') or (r.role_code='BOSS' and m.menu_code='export.details') on conflict do nothing",Map.of());
        }
        ...
        db.exec("""
            ...
            from dim_shop s cross join (values
              ('1瓶精华液','12320JN-1',100),('2瓶精华液','12320JN-11',200),
              ('2瓶精华液','SKUSHUADAN',250),('1瓶精华液+刮痧板','12320JN-GS',300),
              ('刮痧板','12320FJGSS-1',400),('2瓶精华液+刮痧板','12320JNZH',500)
