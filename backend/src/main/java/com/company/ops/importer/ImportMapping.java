...
public final class ImportMapping {
    ...
    public static final Map<String,List<String>> REQUIRED=Map.of("SHOP_ANALYTICS",List.of("biz_date","gmv","order_count"),"PRODUCT_DAILY",List.of("product_id","gmv","order_count","sold_qty","sku_order_count","impressions","clicks","add_to_cart_count"),"ORDER_DETAIL",List.of("order_id","source_status","order_amount","item_qty"),"GMV_MAX_CAMPAIGN",List.of("biz_date","spend","attributed_revenue","attributed_order_count"));
    ...
    public static LocalDate date(String value){
        ...
        for(String pattern:List.of("yyyy-MM-dd","yyyy/M/d","yyyy年M月d日","yyyyMMdd","dd/MM/yyyy","M/d/yyyy"))try{return LocalDate.parse(v,DateTimeFormatter.ofPattern(pattern));}catch(Exception ignored){}
    ...
    public static String status(String raw){
        return switch(normalize(raw)){
            case "unpaid","awaitingpayment","未付款","待付款" -> "UNPAID";
