...
import type { DailyContext, DailyMetricReport, DailySummary, Market } from '../../api/types'
import { useAuth } from '../../stores/auth'
import RequestError from '../../components/RequestError.vue'

type Metric = { plan: keyof DailyMetricReport; actual: keyof DailyMetricReport; label: string }
type TaskRow = { report: DailyMetricReport; metric: Metric; marketName: string; role: string; plan: number; actual: number; gap: number; rate: number; delivery: string }
...
const auth=useAuth(), context=ref<DailyContext>(), rows=ref<DailyMetricReport[]>([]), summaryReports=ref<DailySummary[]>([]), loading=ref(false), saving=ref(false), error=ref<Error|null>(null)
...
function taskRows(reports:DailyMetricReport[]):TaskRow[]{return reports.flatMap(report=>(report.reportType==='EDITOR'?editorMetrics:leadMetrics).map(metric=>{const plan=Number(report[metric.plan]??0),actual=Number(report[metric.actual]??0);return {report,metric,marketName:report.marketName??report.marketCode,role:report.reportType==='EDITOR'?'剪辑':'编导',plan,actual,gap:Math.max(plan-actual,0),rate:plan?actual/plan:0,delivery:report.deliveryResults?.[String(metric.actual)]??''}}))}
const marketTaskRows=computed(()=>taskRows(rows.value))
...
async function load(){
  ...
  try{
    if(tab.value==='summary')summaryReports.value=await api<DailySummary[]>(`/daily-reports/summary?${query({date:selectedDate.value})}`)
...
<template>
  ...
  <el-tabs v-model="tab" class="daily-tabs">
    <el-tab-pane v-if="viewer" label="汇总" name="summary"><section class="surface table-panel"><div class="sheet-title">五区内容日报汇总 <span>按市场汇总各岗位已通过日报的计划与实际数据</span></div><div v-loading="loading" class="table-scroll"><table class="summary-table"><thead><tr><th rowspan="2">地区</th><th v-for="metric in summaryMetrics" :key="metric.label" colspan="2">{{ metric.label }}</th></tr><tr><template v-for="metric in summaryMetrics" :key="`${metric.label}-head`"><th>计划</th><th>实际</th></template></tr></thead><tbody><tr v-for="row in summaryReports" :key="row.marketCode"><td>{{ row.marketName }}</td><template v-for="metric in summaryMetrics" :key="`${row.marketCode}-${metric.label}`"><td>{{ number(Number(row[metric.plan])) }}</td><td>{{ number(Number(row[metric.actual])) }}</td></template></tr><tr v-if="!loading&&!summaryReports.length"><td :colspan="summaryMetrics.length*2+1" class="empty">该日期暂无已通过日报</td></tr><tr v-if="!loading&&summaryReports.length" class="summary-total"><td>合计</td><template v-for="metric in summaryMetrics" :key="`total-${metric.label}`"><td>{{ number(summaryTotal[metric.plan]) }}</td><td>{{ number(summaryTotal[metric.actual]) }}</td></template></tr></tbody></table></div></section></el-tab-pane>
    <el-tab-pane v-for="market in (context?.role==='ADS_BUYER'?[]:(viewer?marketTabs:marketTabs.filter(item=>item.marketCode===ownMarket)))" :key="market.marketCode" :label="`${market.marketName}日报`" :name="`market-${market.marketCode}`"><section class="surface table-panel"><div class="sheet-title">{{ market.marketName }}区日报 <span>{{ departmentReviewer?'部门负责人可审核待处理日报。':'按个人日报展示。' }}</span></div><div v-loading="loading" class="table-scroll"><table class="task-table"><thead><tr><th>地区</th><th>每日任务</th><th>执行岗位</th><th>具体负责人</th><th>计划</th><th>实际</th><th>还差</th><th>完成率</th><th>交付链接 / 编号清单</th><th>截止时间</th><th>状态</th><th v-if="reviewColumn">操作</th></tr></thead><tbody><tr v-for="item in marketTaskRows" :key="`${item.report.id}-${item.metric.actual}`"><td>{{ item.marketName }}</td><td>{{ item.metric.label }}</td><td>{{ item.role }}</td><td>{{ item.report.reporterName }}</td><td>{{ number(item.plan) }}</td><td>{{ number(item.actual) }}</td><td>{{ number(item.gap) }}</td><td>{{ item.plan?percent(item.rate):'—' }}</td><td class="delivery-cell">{{ item.delivery||'—' }}</td><td>{{ submittedAt(item.report.submittedAt) }}</td><td><el-tag size="small" :type="statusType(item.report.submissionStatus)">{{ status(item.report.submissionStatus) }}</el-tag></td><td v-if="reviewColumn"><template v-if="canReview(item.report)"><el-button link type="success" @click="review(item.report,true)">通过</el-button><el-button link type="danger" @click="review(item.report,false)">不通过</el-button></template><span v-else>—</span></td></tr><tr v-if="!loading&&!marketTaskRows.length"><td :colspan="reviewColumn?12:11" class="empty">该日期暂无日报</td></tr></tbody></table></div></section></el-tab-pane>
    <el-tab-pane v-if="viewer||context?.role==='ADS_BUYER'" label="投流日报" name="ads"><section class="surface table-panel"><div class="sheet-title">投手数据｜素材测试、消耗、GMV、ROI、CTR、订单 <span>金额和 ROI 按地区账户币种查看。</span></div><div v-loading="loading" class="table-scroll"><table class="ads-table"><thead><tr><th>地区</th><th>填报人</th><th>状态</th><th v-for="field in adFields" :key="field.key">{{ currencyLabel(field,'') }}</th><th>退回原因</th><th v-if="departmentReviewer">操作</th></tr></thead><tbody><tr v-for="row in rows" :key="row.id"><td>{{ row.marketName }}</td><td>{{ row.reporterName }}</td><td><el-tag size="small" :type="statusType(row.submissionStatus)">{{ status(row.submissionStatus) }}</el-tag></td><td v-for="field in adFields" :key="field.key">{{ adValue(row,field.key) }}</td><td>{{ row.rejectionReason||'—' }}</td><td v-if="departmentReviewer"><template v-if="canReview(row)"><el-button link type="success" @click="review(row,true)">通过</el-button><el-button link type="danger" @click="review(row,false)">不通过</el-button></template><span v-else>—</span></td></tr><tr v-if="!loading&&!rows.length"><td :colspan="19" class="empty">该日期暂无投流日报</td></tr></tbody></table></div><p class="note">同一素材重复测试不累计测试素材数量；测试缺口 = max(计划测试 − 实际测试，0)，ROI 和 CTR 自动计算。</p></section></el-tab-pane>
    <el-tab-pane v-if="contentRole" label="新增日报" name="content-submit"><section class="surface form-panel" v-loading="loading"><div class="sheet-title">{{ context?.role==='EDITOR'?'剪辑':'编导' }}日报｜{{ content.marketCode }} <el-tag size="small" :type="statusType(content.submissionStatus)">{{ status(content.submissionStatus) }}</el-tag></div><el-alert v-if="content.rejectionReason" title="日报已退回" :description="content.rejectionReason" type="error" :closable="false"/><div class="report-table-scroll"><table class="report-form-table"><thead><tr><th>每日任务</th><th>计划</th><th>实际</th><th>还差</th><th>完成率</th><th>交付成果</th></tr></thead><tbody><tr v-for="metric in contentFormMetrics()" :key="metric.label"><td>{{ metric.label }}</td><td><el-input-number v-model="content[metric.plan] as number" :min="0" :precision="0" controls-position="right"/></td><td><el-input-number v-model="content[metric.actual] as number" :min="0" :precision="0" controls-position="right"/></td><td>{{ number(Math.max(Number(content[metric.plan]??0)-Number(content[metric.actual]??0),0)) }}</td><td>{{ Number(content[metric.plan])?percent(Number(content[metric.actual])/Number(content[metric.plan])):'—' }}</td><td><el-input v-model="content.deliveryResults[String(metric.actual)]" maxlength="2000" placeholder="填写链接、编号或交付说明"/></td></tr></tbody></table></div><div class="submit-bar"><span>交付成果由填报人填写；提交成功后自动记录提交时间：{{ submittedAt(content.submittedAt) }}</span><div><el-button :loading="saving" @click="saveContent(false)">暂存</el-button><el-button type="primary" :loading="saving" @click="saveContent(true)">提交日报</el-button></div></div></section></el-tab-pane>
    <el-tab-pane v-if="context?.role==='ADS_BUYER'" label="新增投流日报" name="ads-submit"><section class="surface form-panel" v-loading="loading"><div class="sheet-title">投流日报填报</div><el-tabs v-model="adMarket" type="card" class="market-picker"><el-tab-pane v-for="market in marketTabs" :key="market.marketCode" :label="market.marketName" :name="market.marketCode"/></el-tabs><template v-if="ads[adMarket]"><el-alert v-if="ads[adMarket].rejectionReason" title="该地区日报已退回" :description="ads[adMarket].rejectionReason" type="error" :closable="false"/><div class="metric-form ads-form"><label v-for="field in adFields.filter(field=>!['testGap','roi','ctr'].includes(String(field.key)))" :key="field.key"><span>{{ currencyLabel(field,adMarket) }}</span><el-input-number v-model="ads[adMarket][field.key] as number" :min="0" :precision="field.money?2:0" controls-position="right"/></label><label><span>测试缺口（自动）</span><el-input :model-value="String(Math.max(ads[adMarket].plannedTest-ads[adMarket].actualTest,0))" readonly/></label><label><span>ROI（自动）</span><el-input :model-value="number(ads[adMarket].adSpend?ads[adMarket].adGmv/ads[adMarket].adSpend:0)" readonly/></label><label><span>CTR（自动）</span><el-input :model-value="percent(ads[adMarket].impressions?ads[adMarket].clicks/ads[adMarket].impressions:0)" readonly/></label></div><div class="submit-bar"><span>一名投手每天可按五个地区分别提交。</span><div><el-button :loading="saving" @click="saveAds(false)">暂存</el-button><el-button type="primary" :loading="saving" @click="saveAds(true)">提交{{ marketTabs.find(item=>item.marketCode===adMarket)?.marketName }}日报</el-button></div></div></template></section></el-tab-pane>
  </el-tabs>
</template>
