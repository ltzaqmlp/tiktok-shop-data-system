...
public class DailyReportController {
    ...
    private static final List<Map<String,Object>> EDITOR_TASKS=List.of(
        ...
        p("taskCode","REWORK_ACCEPTANCE","taskName","返工验收","planField","plannedReworkAcceptance","actualField","actualReworkAcceptance")
    );
    ...
    private static Map<String,Object> blank(LocalDate date,String type,String market){var value=p("reportDate",date.toString(),"reportType",type,"marketCode",market,"submissionStatus","DRAFT","rejectionReason","","notes","","blockers","","deliveryResults",new LinkedHashMap<>(),"editorTaskResults",new LinkedHashMap<>(),"submittedAt",null);for(String key:EDITOR)value.put(key,0);for(String key:LEAD)value.put(key,0);for(String key:ADS)value.put(key,key.equals("adSpend")||key.equals("adGmv")?BigDecimal.ZERO:0);value.put("testGap",0);value.put("roi",BigDecimal.ZERO);value.put("ctr",BigDecimal.ZERO);return value;}
    ...
    static Map<String,Object> metrics(Map<String,Object> body,String type,boolean required){
        ...
        for(String key:fields)values.put(camelToSnake(key),(key.equals("adSpend")||key.equals("adGmv"))?money(body,key,required):number(body,key,required));values.put("notes",Api.text(body,"notes",4000,required));values.put("blockers",Api.text(body,"blockers",4000,required));return values;
    }
