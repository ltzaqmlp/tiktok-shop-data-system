...
- **Shop Analytics** 改为数据看板每日核心指标的主数据源：GMV、订单数、商品成交件数、SKU 订单数、退款金额、商品访客数、转化率。
...
- 订单明细额外保存 `Sku Quantity of return` 和 `Order Refund Amount`，用于核对；由于订单文件缺少权威退款发生日期，数据看板的每日退款金额仍以 Shop Analytics 为准。
