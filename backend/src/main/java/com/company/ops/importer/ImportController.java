...
public class ImportController {
    ...
    @PostMapping public Object upload(@RequestParam String sourceType,@RequestParam String marketCode,@RequestParam long shopId,@RequestParam(required=false)String bizDate,@RequestParam MultipartFile file,@RequestParam(defaultValue="false")boolean force,HttpServletRequest req)throws Exception{
        req.setAttribute("auditAction","IMPORT_START");req.setAttribute("auditSummary",p("sourceType",sourceType,"marketCode",marketCode,"shopId",shopId,"filename",file.getOriginalFilename()));
        ...
        String id=service.submit(sourceType,marketCode,shopId,bizDate,file,Identity.uid(req),force);req.setAttribute("targetId",id);return Api.ok(req,p("id",id));
    }
