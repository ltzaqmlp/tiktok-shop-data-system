...
function requireNotes(submit:boolean,notes:string){if(!submit||notes.trim())return true;ElMessage.warning('备注/卡点不能为空');return false}
async function load(){
...
onMounted(async()=>{try{context.value=await api<DailyContext>('/daily-reports/context');if(!viewer.value)tab.value=context.value.role==='ADS_BUYER'?'ads':context.value.role==='DIRECTOR'?'content-submit':simpleRole.value?'simple-submit':'market-'+ownMarket.value;await load()}catch(e){error.value=e as Error}})
...
<template>
  ...
  <el-tabs v-model="tab" class="daily-tabs">
     ...
     <el-tab-pane v-if="viewer" label="运营日报" name="ops"><section class="surface table-panel"><div class="sheet-title">运营日报 <span>运营提交后由部门负责人审核，审核通过后展示给老板。</span></div><div v-loading="loading" class="table-scroll"><table class="simple-table"><thead><tr><th>填报人</th><th>备注/卡点</th><th>提交时间</th><th>状态</th><th v-if="departmentReviewer">操作</th></tr></thead><tbody><tr v-for="row in rows" :key="row.id"><td>{{ row.reporterName }}</td><td class="notes-cell">{{ row.notes||'—' }}</td><td>{{ submittedAt(row.submittedAt) }}</td><td><el-tag size="small" :type="statusType(row.submissionStatus)">{{ status(row.submissionStatus) }}</el-tag><div v-if="row.rejectionReason" class="reason">{{ reasonText(row.rejectionReason) }}</div></td><td v-if="departmentReviewer"><template v-if="canReview(row)"><el-button link type="success" @click="review(row,true)">通过</el-button><el-button link type="danger" @click="review(row,false)">不通过</el-button></template><span v-else>—</span></td></tr><tr v-if="!loading&&!rows.length"><td :colspan="departmentReviewer?5:4" class="empty">该日期暂无运营日报</td></tr></tbody></table></div></section></el-tab-pane>
     <el-tab-pane v-if="viewer" label="技术日报" name="tech"><section class="surface table-panel"><div class="sheet-title">技术日报 <span>技术提交后由部门负责人审核，审核通过后展示给老板。</span></div><div v-loading="loading" class="table-scroll"><table class="simple-table"><thead><tr><th>填报人</th><th>备注/卡点</th><th>提交时间</th><th>状态</th><th v-if="departmentReviewer">操作</th></tr></thead><tbody><tr v-for="row in rows" :key="row.id"><td>{{ row.reporterName }}</td><td class="notes-cell">{{ row.notes||'—' }}</td><td>{{ submittedAt(row.submittedAt) }}</td><td><el-tag size="small" :type="statusType(row.submissionStatus)">{{ status(row.submissionStatus) }}</el-tag><div v-if="row.rejectionReason" class="reason">{{ reasonText(row.rejectionReason) }}</div></td><td v-if="departmentReviewer"><template v-if="canReview(row)"><el-button link type="success" @click="review(row,true)">通过</el-button><el-button link type="danger" @click="review(row,false)">不通过</el-button></template><span v-else>—</span></td></tr><tr v-if="!loading&&!rows.length"><td :colspan="departmentReviewer?5:4" class="empty">该日期暂无技术日报</td></tr></tbody></table></div></section></el-tab-pane>
...
.report-form-table + .report-note>span::after{content:' *';color:var(--el-color-danger)}
