...
public class ModuleRegistrar implements ApplicationRunner {
    ...
    @Override @Transactional public void run(ApplicationArguments ignored){
        ...
        if(bootstrap){
            ...
            db.exec("insert into sys_role_menu(role_id,menu_id) select r.id,m.id from sys_role r cross join sys_menu m where r.role_code in ('ADMIN','BOSS') or (r.role_code in ('DEPT_HEAD','OPS','ADS_BUYER','DIRECTOR') and m.menu_code='dashboard.overview') on conflict do nothing",Map.of());
        }
