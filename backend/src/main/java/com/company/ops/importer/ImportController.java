...
public class ImportController {
    ...
    @PostMapping public Object upload(@RequestParam String sourceType,@RequestParam String marketCode,@RequestParam long shopId,@RequestParam(required=false)String bizDate,@RequestParam("file")List<MultipartFile> files,@RequestParam(defaultValue="false")boolean force,HttpServletRequest req)throws Exception{
        Api.require(!files.isEmpty()&&files.size()<=10,"一次最多导入 10 个文件");Api.require(files.stream().mapToLong(MultipartFile::getSize).sum()<=500L*1024*1024,"批量文件总大小不能超过 500 MB");
        ...
        var ids=new ArrayList<String>();for(var file:files)ids.add(service.submit(sourceType,marketCode,shopId,bizDate,file,Identity.uid(req),force,files.size()>1));req.setAttribute("targetId",String.join(",",ids));return Api.ok(req,p("ids",ids,"id",ids.getFirst()));
    }
