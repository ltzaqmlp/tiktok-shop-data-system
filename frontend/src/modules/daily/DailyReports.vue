...
const report=reactive<DailyReport>({reportDate:reportDate.value,groupName:'',roleMarket:'',todayFocus:'',keyResult:'',needBossSupport:'',tomorrowFocus:'',submissionStatus:'DRAFT',reporterName:'',tasks:[blankTask()]})
...
function applyMine(value:DailyReport){const tasks=value.tasks.length===0?[blankTask()]:value.tasks.length===1?value.tasks:[{...blankTask(),workModule:value.tasks.map(task=>task.workModule).filter(Boolean).join('、'),workDetail:value.tasks.map(task=>task.workDetail).filter(Boolean).join('\n'),planDelivery:value.tasks.map(task=>task.planDelivery).filter(Boolean).join('\n'),actualResult:value.tasks.map(task=>task.actualResult).filter(Boolean).join('\n'),completionStatus:value.tasks.some(task=>task.completionStatus==='已阻塞')?'已阻塞':value.tasks.some(task=>task.completionStatus==='进行中')?'进行中':value.tasks.every(task=>task.completionStatus==='已完成')?'已完成':'未开始',issueNextStep:value.tasks.map(task=>task.issueNextStep).filter(Boolean).join('\n'),resultLink:value.tasks.map(task=>task.resultLink).filter(Boolean).join('\n')}];Object.assign(report,{...value,reportDate:reportDate.value,tasks:tasks.map(task=>({...task}))})}
...
<template>
  ...
  <el-tabs v-model="tab" class="sheet-tabs">
    ...
    <el-tab-pane :label="leader?'团队每日明细':'我的日报记录'" name="details">
      <section class="surface excel-sheet"><div class="excel-title">TikTok 团队｜个人每日工作明细</div><div class="excel-note">每人每天一行完整日报；成员只能读取自己的记录。</div>
    ...
    <el-tab-pane label="提交日报" name="submit">
      <section class="surface submit-sheet"><div class="excel-title">TikTok 团队｜个人每日工作明细</div><p class="excel-note">早上写计划，收工前补结果。暂存仅本人可见，提交后领导看到完全相同的字段。</p>
        ...
        <div class="task-head"><h2>日报明细</h2><span class="muted small">每人每天填写一份完整日报</span></div>
        <article v-if="report.tasks[0]" class="task-form"><div class="task-form-title"><strong>今日工作汇总</strong></div><div class="task-grid"><label>工作模块<el-input v-model="report.tasks[0].workModule" maxlength="100" placeholder="如：内容审核、素材制作"/></label><label>完成状态<el-select v-model="report.tasks[0].completionStatus"><el-option label="未开始" value="未开始"/><el-option label="进行中" value="进行中"/><el-option label="已完成" value="已完成"/><el-option label="已阻塞" value="已阻塞"/></el-select></label><label class="wide">今日具体工作<el-input v-model="report.tasks[0].workDetail" type="textarea" :rows="5" maxlength="2000" show-word-limit placeholder="请把今天的工作一次性写完整，可分行描述多个事项"/></label><label>计划交付<el-input v-model="report.tasks[0].planDelivery" type="textarea" :rows="3" maxlength="2000" show-word-limit placeholder="如：脚本 5 条、素材 3 组"/></label><label>实际完成 / 结果<el-input v-model="report.tasks[0].actualResult" type="textarea" :rows="3" maxlength="2000" show-word-limit/></label><label>成果链接 / 位置<el-input v-model="report.tasks[0].resultLink" maxlength="2000" placeholder="链接或文件夹位置（可选）"/></label><label class="wide">问题及下一步<el-input v-model="report.tasks[0].issueNextStep" type="textarea" :rows="3" maxlength="2000" show-word-limit placeholder="可一次性写明问题、处理人和时间"/></label></div></article>
